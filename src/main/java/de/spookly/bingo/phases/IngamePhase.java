...
import de.spookly.bingo.bossbar.BossBarManager;
import de.spookly.bingo.bossbar.BossBarSection;
import de.spookly.bingo.bossbar.BossComponentHelper;
...
public class IngamePhase extends AbstractGamePhase {
	...
	public IngamePhase(SpooklyBingoPlugin spooklyBingoPlugin) {
	...
	public void startPhase() {
		...
		spooklyBingoPlugin.getWorldManager().setWorldSettingsForGameWorlds(Objects.requireNonNull(Bukkit.getWorld("world")));
		if (GameData.getActiveNether())
			spooklyBingoPlugin.getWorldManager().setWorldSettingsForGameWorlds(Objects.requireNonNull(Bukkit.getWorld("world_nether")));
		...
		Spookly.getOnlinePlayers().forEach(splayer -> {
			...
			BingoPlayerStats stats = spooklyBingoPlugin.getPlayerStatisticsManager().getPlayerStatistics(splayer);
			...
			SpooklyBingoPlugin.getSpooklyBingoPlugin().getScoreboardManager().setUserScoreboard(player);
...
//            );
			...
			if (counter == 0) {
				...
				Team winner = teamItems.entrySet().stream()
						...
						.get(0).getKey();
				spooklyBingoPlugin.getEndingPhase().teamWin(winner);
