...
public class GameData {
    ...
    @Getter private static Integer teamSize = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.teamSize", 2);
    @Getter private static Integer teamAmount = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.teamAmount", 9);
    @Getter private static Boolean isHunger = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.hungerEnabled", true);
    @Getter private static Boolean stats = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.stats", false);
    @Getter private static Integer startTime = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.startTime", 60);
    @Getter private static Integer maxGameTime = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.maxGameTime", 7200);
    @Getter private static Integer worldSize = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.worldSize", 13000);
    @Getter private static Integer playerSpawnSize = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.playerSpawnSize", 10000);
    @Getter private static Boolean activeNether = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.activeNether", true);
    @Getter private static Integer itemsAmount = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.itemsAmount", 9);
    @Getter private static String databaseTableName = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.database.tableName", "spookly_bingo_stats");
...
//    @Getter private static Integer minPlayerToStartGame = 2;
    ...
    @Getter private static List<String> itemPool = SpooklyBingoPlugin.getSpooklyBingoPlugin().getItemsConfig().getOrSetDefault("config.itempool", new ArrayList<String>(
            Arrays.asList(
    ...
    @Getter private static List<String> noSpawnBiomes = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.game.noSpawnBiomes", new ArrayList<String>(
            Arrays.asList(
    ...
    @Getter private static String configVer = SpooklyBingoPlugin.getSpooklyBingoPlugin().getGeneralConfig().getOrSetDefault("config.version", "1.2");
